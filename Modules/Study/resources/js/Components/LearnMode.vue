...
// Adaptive queue: start with all terms, re-add wrong ones
...
const showCard = ref(true); // flip between question and answer

const current = computed(() => queue.value[currentIndex.value]);
const progress = computed(() => {
  const done = props.terms.length - wrongQueue.value.length;
  return Math.round((done / props.terms.length) * 100);
});
  ...
  <div v-else>
    ...
    <div class="mb-4">
      <div class="flex justify-between text-sm text-gray-500 mb-2">
        ...
        <span v-else class="text-green-600">{{ progress }}% ✓</span>
      </div>
    ...
    <div class="flex gap-3">
      ...
      <button v-else @click="next"
        :class="['flex-1 py-3 rounded-xl font-medium text-white transition', isCorrect ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-800 hover:bg-gray-900']">
        {{ currentIndex < queue.length - 1 ? 'Növbəti →' : (wrongQueue.length > 0 ? `${wrongQueue.length} yanlışa qayıt →` : 'Bitir') }}
      </button>
    </div>
  </div>
</template>
